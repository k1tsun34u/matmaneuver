...
export default class CartCard {
	constructor(redirectUrl, storageKey, name, price, quantity, productId) {
		...
		this._elIncQty.addEventListener('click', () => {
			Carts.Add('ACTIVE', productId).then(r => {
				const firstIndex = this._elQty.innerHTML.indexOf(' ');
				const lastIndex = this._elQty.innerHTML.lastIndexOf(' ');
				
				const curQty = parseInt(this._elQty.innerHTML.substring(firstIndex + 1, lastIndex));
				this.qty = curQty + 1;
			});
		});
		...
		this._elDecQty.addEventListener('click', () => {
			...
			Carts.Dec('ACTIVE', productId).then(r => {
				this.qty = curQty - 1;
			});
		});
		...
		this._elRemove.classList.add('red-bg', 'card-remove');
		this._elRemove.innerHTML = 'Убрать';
		this._elRemove.addEventListener('click', () => {
			Carts.RemoveItem('ACTIVE', productId).then(r => {
				this._base.remove();
	...
	set qty(quantity) {
		this._elQty.innerHTML = `Кол-во: ${quantity} шт`
	}
